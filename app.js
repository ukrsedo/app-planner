  ...
  function aiLabels(){
    return {
      en:{
        ...
        passed:'Contract signature target already passed',
        ...
        req:'Requirement',dept:'Department',needBy:'Contract signature',start:'Procurement start',timing:'Timing',criticality:'Criticality',amount:'Value',
      ...
      uk:{
        ...
        total:'Усього потреб',
        passed:'Цільова дата підписання договору вже минула',
        ...
        req:'Потреба',dept:'Підрозділ',needBy:'Підписання договору',start:'Початок закупівлі',timing:'Строк',criticality:'Критичність',amount:'Вартість',
      ...
      pt:{
        ...
        passed:'Data-alvo de assinatura do contrato já ultrapassada',
        ...
        req:'Necessidade',dept:'Departamento',needBy:'Assinatura do contrato',start:'Início da compra',timing:'Prazo',criticality:'Criticidade',amount:'Valor',
  ...
  function renderDeterministicAiSummary(rows, aiData=null){
    ...
    const readiness=(incomplete>0||missingValue>0) ? `
      <div class="report-subsection">
        ...
        <table class="report-table compact readiness-table">
          <tbody>
    ...
    root.innerHTML=`
      <div class="app-ai-report">
        ...
        <section class="report-section">
          ...
          <div class="dashboard-grid">
            <div class="dashboard-card">
              ...
              <table class="report-table dashboard-table"><thead><tr><th>${escapeHtml(L.metric)}</th><th>${escapeHtml(L.value)}</th></tr></thead><tbody>${metricRows}</tbody></table>
            </div>
            <div class="dashboard-card spend-card">
              <h3>${escapeHtml(L.spend)}</h3>
              <table class="report-table spend-table"><thead><tr><th>${escapeHtml(L.currency)}</th><th>${escapeHtml(L.value)}</th></tr></thead><tbody>${spendRows}</tbody></table>
            </div>
          </div>
        </section>
        ...
        <section class="report-section">
          ...
          <table class="report-table pipeline-table"><thead><tr><th>${escapeHtml(L.window)}</th><th>${escapeHtml(L.count)}</th><th>${escapeHtml(L.interpretation)}</th></tr></thead><tbody>${pipelineRows}</tbody></table>
        </section>
        ...
        <section class="report-section">
          ...
          <div class="concentration-grid">
            <div>
              ...
              <table class="report-table compact concentration-table"><thead><tr><th>${escapeHtml(L.name)}</th><th>${escapeHtml(L.requirements)}</th></tr></thead><tbody>${deptRows}</tbody></table>
            </div>
            <div>
              <h3>${escapeHtml(L.topCategories)}</h3>
              <table class="report-table compact concentration-table"><thead><tr><th>${escapeHtml(L.name)}</th><th>${escapeHtml(L.requirements)}</th></tr></thead><tbody>${catRows}</tbody></table>
            </div>
          </div>
